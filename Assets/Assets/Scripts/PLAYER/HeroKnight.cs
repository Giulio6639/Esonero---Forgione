...
public class HeroKnight : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Input.GetKeyDown("left shift") && !m_rolling && !m_blocking && rollCooldownTimer <= 0f)
        {
            m_rolling = true;
            m_body2d.gravityScale = defaultGravity;
    ...
    public void ThrowItem(GameObject itemPrefab)
    {
        Vector3 spawnPos = transform.position + new Vector3(m_facingDirection * 0.5f, 0.5f, 0);
        GameObject thrownItem = Instantiate(itemPrefab, spawnPos, Quaternion.identity);

        HolyWaterBottle bottle = thrownItem.GetComponent<HolyWaterBottle>();
        if (bottle != null)
        {
            bottle.Initialize(m_facingDirection);
        }
    }

    public void ApplyPowerUp(float multiplier, float duration)
    {
        StartCoroutine(PowerUpRoutine(multiplier, duration));
    }
    ...
    private IEnumerator PowerUpRoutine(float multiplier, float duration)
    {
        ...
        Color powerUpColor = new Color(1f, 0.92f, 0.016f, 1f);
        float flashSpeed = 8f;
        ...
        while (elapsed < duration)
        {
            ...
            m_spriteRenderer.color = Color.Lerp(Color.white, powerUpColor, t);
        ...
        Debug.Log("Power up terminato. Statistiche tornate alla norma.");
    }
}